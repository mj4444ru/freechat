...
namespace FCEvent {
    ...
    export var handlers: {[key: string]: Function} = {};

    $(document).on('click', '[data-fc-click]', function (event) {
        event.preventDefault();
        const $target = $(event.currentTarget);
        const handlerName = $target.attr('data-fc-click') as string;
        if (handlerName && handlers[handlerName]) {
            return handlers[handlerName].call(this, event, $target);
        } else {
            const eventName = 'fc-click-' + ($(event.currentTarget).attr('data-fc-click') as string);
...
namespace FCString {
    ...
    export function isValidErrorText(text: any): text is string {
...
namespace FCData {
    ...
    export function isAuthData(data: any): data is AuthData {
...
namespace FCAjax {
    ...
    export function isRedirect(jqXHR: JQueryXHR): string {
...
namespace FCPjax {
    ...
    if ('pushState' in window.history) {
        let fbCount = 0;
        $(function () {
            ($.fancybox as any).defaults.hash = false;
        });
        ...
        $(document).on('beforeShow.fb', function(e, instance, slide) {
            fbCount++;
            const opts = instance ? (instance.current ? instance.current.opts : instance.opts) : {};
            const hash = opts.$orig ? opts.$orig.data('fancybox') : '';
            const href = window.location.pathname + window.location.search + (hash ? '#' + hash : window.location.hash);
            let pushState = false;
            if (window.history.state) {
                pushState = !window.history.state.fbindex || window.history.state.fbindex < fbCount;
            } else {
                pushState = !hash || ('#' + hash) != window.location.hash;
            }
            if (pushState) {
                window.history.pushState({fb: true, fbhref: href, fbindex: fbCount}, document.title, href);
            }
            const title = opts.$orig ? opts.$orig.text() : (opts.title ? opts.title : '');
            if (title) {
        ...
        $(document).on('afterClose.fb', function(e, instance, slide) {
            fbCount--;
            if (window.history.state.fbindex && window.history.state.fbindex > fbCount) {
                history.go(fbCount - window.history.state.fbindex);
            }
        });
...
namespace FCRequired {
    ...
    export function validateRequiredInput($input: JQuery) {
...
namespace FCPopover {
    ...
    FCEvent.handlers.popover = function (event: JQueryEventObject, $target: JQuery) {
    ...
    export function showPopover(config: PopoverData) {
...
namespace FCTopMenu {
    ...
    export function updateActive() {
        ...
        const $activeNavLink = $topMenu.find('a.active');
...
namespace FCLoginForm {
    'use strict';

    FCEvent.handlers.loginForm = function (event: JQueryEventObject, $target: JQuery) {
        const $form = $target.closest('form');
        const login = FCRequired.validateRequiredInput($form.find('input.login'));
        const password = FCRequired.validateRequiredInput($form.find('input.password'));
        ...
        function ajaxDone(data: FCData.AuthData | boolean | string) {
    ...
    FCEvent.handlers.loginFormChange = function (event: JQueryEventObject, $target: JQuery) {
        $target.parents('form').find('.bad-password:visible').fadeOut();
    };
}

namespace FCGuestRegisterForm {
    'use strict';
    ...
    FCEvent.handlers.guestForm = function (event: JQueryEventObject, $target: JQuery) {
...
namespace FCMain {
    ...
    export const defErrorMessage = 'Error';

    FCEvent.events.onLogin = function (data: FCData.AuthData) {
